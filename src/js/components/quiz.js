...
const quizTemplate = (data = [], dataLength = 0, options) => {
    ...
    if (item.type === 'checkbox') {
      ...
      <li class="quiz-question__item">
        <img src="img/sneaker.jpg" alt="sneaker">
	...
	nextQuestion() {
		...
		if (this.valid()) {
			if (this.counter + 1 < this.dataLength) {
				...
				if ((this.counter + 1 == this.dataLength)) {
					// this.$el.querySelector('.quize-bottom').insertAdjacentHTML('beforeend', `<button type="button" data-send>${this.options.sendBtnText}</button>`)
        ...
        document.querySelector('.quiz__questions').style.display = 'none'
...
window.quiz = new Quiz('.quiz-form .quiz__questions', quizData, {
